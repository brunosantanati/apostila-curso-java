...
Portanto um passo na direção do encapsulamento seria declarar as variáveis de instância da classe como privadas e permitir o acesso a elas apenas através de métodos públicos. O método para recuperar o valor da variável tem o padrão de nomenclatura get<NomeDaPropriedade>, e o método para atribuir um valor a variável tem o padrão de nomenclatura set<NomeDaPropriedade>. Esses tipos de método são conhecidos no mundo Java como getters e setters. Uma observação importante seria que o padrão de nomenclatura geralmente adotado para recuperar valores de campos do tipo boolean é is<NomeDaPropriedade>. Vejamos como ficaria nossa classe com essa alteração:
